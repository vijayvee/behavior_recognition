...
def train_batch_videos(n_train_batches, n_epochs,
    ...
    with tf.Session().as_default() as sess:
        ...
        predictions,loss,top_classes,input_video_ph,input_video_ph_norm, saver = get_preds_loss_tfrecords(ground_truth=labels,
        ...
        if input_mode=='rgb':
            ...
            try:
                for i in tqdm(range(0,n_iters),desc='Training I3D on mice train set...'):
                    curr_loss,top_class_batch,videos_unnorm, labels, _= sess.run([loss,
                                                                                   top_classes,
                                                                                   step, input_video_ph,
                                                                                   ground_truth,
                                                                                   ])
                    correct_preds += list(top_class_batch==labels).count(True)
                    ...
                    if i%print_every==0:
                        print 'Iteration-%s Current training loss: %s Current training accuracy: %s'%((i+1),
                                                                                                    ...
                                                                                                    train_acc)
                    if i%action_every==0:
                        print_preds_labels(top_class_batch, labels)
                    if i%save_every==0:
                ...
                print "TfRecords weren't written beyond :( restart training from ckpt"
            finally:
                coord.request_stop()
            coord.join(threads)
