#!/usr/bin/python
"""Script to predict actions from videos in batches"""
...
def infer_video(video_path,input_mode='rgb', print_every=10,
                show_output=True,action_every=50, num_classes=9,
    ...
    with tf.Session().as_default() as sess:
        ...
        if show_output:
            out_vid = cv2.VideoWriter('%s_predictions_i3d.avi'%(video_path),
                            cv2.VideoWriter_fourcc('M','J','P','G'),
                            30,(_IMAGE_SIZE,_IMAGE_SIZE))
        ...
        if input_mode=='rgb':
            ...
            start = time.time()
            for ii in tqdm(range(0,VIDEO_LENGTH,n_frames),desc='Inferring on sample'):
                frames, shape = get_video_chunk_cv2(video_path,ii,
                                                    2*n_frames)
                frames_packed = pack_frames(frames,
                                    batch_size=batch_size,
                                    n_frames=n_frames
                                    )
                feed_dict = {videos: frames_packed}
                ...
                if show_output:
                    for fr_ind in range(n_frames,2*n_frames):
                        img = frames[fr_ind]
                        cv2.putText(img, text_labels[fr_ind-n_frames], (90,50), 
